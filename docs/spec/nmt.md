...
Below, we focus on the Merkle range proof where the range is `[start, end)`.
...
- Compute the tree root `T'` using the `leaves` and the `proof.nodes`.
  ...
  To compute the tree root, the [namespaced hash function](#namespaced-hash) should be utilized.
