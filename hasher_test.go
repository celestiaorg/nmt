...
func TestComputeRange(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		minNsV, maxNsV := computeNsRangeVerbose(tc.leftMinNs, tc.leftMaxNs, tc.rightMinNs, tc.rightMaxNs, tc.ignoreMaxNs, precomputedMaxNs)
		assert.True(t, bytes.Equal(minNs, minNsV))
		assert.True(t, bytes.Equal(maxNs, maxNsV))
	}
}
