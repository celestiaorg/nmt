...
type NmtHasher interface {
	...
	// tree.
	// It additionally returns the hashed value.
	EmptyRoot() ([]byte, []byte)

	// HashLeaf defines how a leaf is hashed.
	// It additionally returns the hashed value.
	HashLeaf(leaf []byte) ([]byte, []byte)

	// HashNode defines how a inner node is hashed.
	// It additionally returns the hashed value.
	HashNode(l, r []byte) ([]byte, []byte)
...
func (n *DefaultNmtHasher) EmptyRoot() ([]byte, []byte) {
...
//nolint:errcheck
func (n *DefaultNmtHasher) HashLeaf(leaf []byte) ([]byte, []byte) {
	h := n.Hash
	h.Reset()
...
func (n *DefaultNmtHasher) HashNode(l, r []byte) ([]byte, []byte) {
	h := n.Hash
	h.Reset()
